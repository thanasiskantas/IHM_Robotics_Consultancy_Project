...
function [motor_angle_trajectory] = getTrajectory_shaped2(start_pose, goal_pose)
    ...
    % object infor
    L = 3.8; % square side length
    square_diagonal = L * sqrt(2);
    
    % finger width
    W = 2.05*2; % finger_width = finger_width + 2 * rubber_width
    FL = 20; % finger_length
    W_high_friction = W;
    
    % base seperation
    d = 3.25+W; % base_seperate
    base_left = [-d/2, 0];
    base_right = [d/2, 0];
    ...
    %% without rotation or rotation between -90 to 90 && rotate 180
        ...
        if goal_angle > 0 && goal_angle <= 90 % 0 to 90
            ...
            if goal_angle > 45
                ...
                if goal_angle ~= 90
                    ...
                    % slide
                    motor_angles = get_alpha_for_slide(centre2, centre1, base_left, base_right, base_d, FL, W_high_friction, L);
                    sz = size(motor_angles);
                    motor_angles_part = motor_angles(1:(sz(1)-1),:);

                    motor_angle_trajectory = [motor_angle1(1,2),0,high_fric,high_fric; % rotate 90
                                            motor_angles_part; % slide
                                            0, motor_angle2(1,1),low_fric,high_fric; % slide
                                            0, motor_angle2(2,1),high_fric,high_fric % rotate goal-90
                                            ];
                    disp(rad2deg(motor_angle_trajectory));
                else
                    disp("90")
                    % slide
                    goal_pos = [goal_pos(1),goal_pos(2),goal_pos(3)];
                    centre1 = [centre1(1),centre1(2), goal_pose(3)];
                    motor_angles = get_alpha_for_slide(goal_pos, centre1, base_left, base_right, base_d, FL, W_high_friction, L);
                    sz = size(motor_angles);
                    motor_angles_part = motor_angles(1:(sz(1)-1),:);
                    disp("motor_angles")
                    disp(rad2deg(motor_angles))

                    if motor_angles(sz(1),1) ~= 0
                        motor_angle_trajectory = [motor_angle1(1,2),0,high_fric,high_fric; % rotate 90
                                                motor_angles_part(2,:);
                                                pi/2,0,high_fric,low_fric];%slide motor angle
                        disp("90 degree")
                        disp(motor_angle_trajectory)
                    else
                        motor_angle_trajectory = [motor_angle1(1,2),0,high_fric,high_fric; % rotate 90
                                                motor_angles_part(2,:); % slide
                                                0,pi/2,low_fric,high_fric];
                        %slide motor angle
                        disp("90 degree")
                        disp(motor_angle_trajectory)
                    end
            ...
            elseif goal_angle <= 45
                ...
                sz = size(motor_angles);
                motor_angles_part = motor_angles(1:(sz(1)-1),:);
                motor_angle_trajectory = [motor_angles_part;
        ...
        elseif goal_angle == 0 % no rotation
            ...
            if motor_angles(sz(1),1) ~= 0
                ...
                motor_angle_trajectory = [motor_angles_part;
                                     0,pi/2,low_fric,high_fric];%slide motor angle
                disp(motor_angle_trajectory)
            end
        ...
        elseif abs(goal_angle) == 180 % rotate 180
            ...
            [motor_angle2, centre2] = get_L2R_Rotate_StartPos(square_diagonal, rotate2_start_pos, d, W, L, base_left, base_right, 0);
            % slide
            motor_angles = get_alpha_for_slide(goal_pose, centre2, base_left, base_right, base_d, FL, W_high_friction, L);
            ...
            motor_angle_trajectory = [motor_angle1(1,2),0,high_fric,high_fric; % rotate 90
                                    ...
                                    motor_angle2(1,2),0,high_fric,high_fric; % rotate 90
                                    motor_angles; % slide
                                    pi/2,0,high_fric,low_fric; % slide
                                    ];
            disp(rad2deg(motor_angle_trajectory));
...
% three slide ------------------
    ...
    centre_to_baseR = sqrt((base_right(1)-start_pose(1))^2 + (start_pose(2))^2);
    d_angle = asin((L/2+W_high_friction/2)/(centre_to_baseR/sin(pi/2)));
    right_motor_angle_mid1 = alpha2_mid1-d_angle;
    right_motor_angle_mid2 = alpha2_mid2-d_angle;
    right_motor_angle_end = alpha2_end-d_angle;

    centre_to_baseL = sqrt((base_left(1)-goal_pose(1))^2 + (goal_pose(2))^2);
    d_angle = asin((L/2+W_high_friction/2)/(centre_to_baseL/sin(pi/2)));
    left_motor_angle_mid1 = alpha1_mid1-d_angle;
    left_motor_angle_mid2 = alpha1_mid2-d_angle;
    left_motor_angle_end = alpha1_end-d_angle;
    
    % disp("check slide angle")
    % disp(right_motor_angle_mid1)
    % disp(right_motor_angle_mid2)
    % disp(right_motor_angle_end)
    % disp(left_motor_angle_mid1)
    % disp(left_motor_angle_mid2)
    % disp(left_motor_angle_end)
    % disp("check complete")

    motor_angles = [0 left_motor_angle_mid1 low_fric high_fric;
                    right_motor_angle_mid2 0 high_fric low_fric;
                    0 left_motor_angle_end low_fric high_fric];
